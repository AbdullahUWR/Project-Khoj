...
public class VIDEUIManager1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (VD.isActive) //If there is a dialogue active
        {
            ...
            if (!data.pausedAction && data.isPlayer)
            {
                ...
                {
                    currentChoices[i].color = Color.black;
                    if (i == data.commentIndex) currentChoices[i].color = Color.blue;
                }
            }
        }
    ...
    public void SetOptions(string[] choices)
    {
        ...
        for (int i = 0; i < choices.Length; i++)
        {
            ...
            newOp.GetComponent<RectTransform>().anchoredPosition = new Vector2(0, 0 - (30 * i));
            newOp.GetComponent<RectTransform>().localScale = new Vector3(1, 1, 1);
            newOp.GetComponentInChildren<Text>().text = choices[i];
            newOp.SetActive(true);

            newOp.GetComponentInChildren<Text>().color = Color.black;
            currentChoices.Add(newOp.GetComponentInChildren<Text>());
